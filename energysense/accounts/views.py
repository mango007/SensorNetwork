...
def login_user(request):
    ...
    if request.method == "POST":
        ...
        if user is not None:
            ...
            resp = {"login":False, "errors":errors}
            return HttpResponse(status=200, content_type='application/json', content=json.dumps(resp))
    return render_to_response("accounts/login.html", {'errors': errors}, RequestContext(request))
...
def register(request):
    ...
    if request.method == "POST":
        ...
        if not errors:
            ...
            resp = {"login":False, "errors":errors}
            return HttpResponse(status=200, content_type='application/json', content=json.dumps(resp))
    return render_to_response("accounts/register.html", {'errors': errors}, RequestContext(request))
